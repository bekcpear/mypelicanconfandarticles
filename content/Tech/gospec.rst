...
然后 case 将实际的类型 :code:`T` 与表达式 :code:`x` 的动态类型进行匹配。与类型断言一样， :code:`x` 必须是 `接口类型`_ ，并且在 case 中的每一个非接口类型 :code:`T` 必须实现了 :code:`x` 的类型。在类型开关中， case 的类型必须都是 `不同的`_ 。
...
TypeSwitchGuard 可能会包含一个 `短变量声明`_ 。当用了这种形式的话，变量会在每个子句的 TypeSwitchCase 末尾的隐式 `块`_ 中被声明。在仅列出一个类型的 case 子句中，变量类型就是这个类型；否则，变量类型为 TypeSwitchGuard 中表达式的类型。

除了类型外， case 可以使用预声明的标识符 `nil`_ ；当 TypeSwitchGuard 中的表达式为 :code:`nil` 接口值时，这个 case 会被选中。最多只可以有一个 :code:`nil` case。
...
1. 对于语句中的所有 case 来说，其接收操作和信道的信道操作数以及发送语句右侧的表达式会在进入 "select" 语句时以源码的顺序被执行仅一次。结果是需要接收或发送的信道集，以及对应的需要发送的值。无论选择哪个（如果有）通信操作进行，在这个求值中的任何副作用都会发生。 RecvStmt 左侧的带短变量声明或赋值的表达式还不会被求值。
2. 如果可以发生一个或多个通信，通过统一的伪随机选择确定一个来进行。否则，如果有一个默认的 case，那么这个 case 会被选择。如果没有默认的 case，那么这个 "select" 语句会阻塞，直到至少发生了一个通信。
