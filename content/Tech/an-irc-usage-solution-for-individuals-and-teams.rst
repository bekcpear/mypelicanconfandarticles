...
.. attention::
  ...
  使用客户端连接上 ZNC 后，如果发现提示无法连接到 Libera.Chat （或其它）网络，如果原因是 IP 受限需要 SASL
  认证的话，这个时候可以选择通过对应网络提供的 Web 端先登录进去（如 https://web.libera.chat/
  ），然后参考各网络的 SASL 文档并设置完成，再启用 ZNC 的 `SASL`_ 模块，并配置后即可成功登录。
